...
client_id: 'ATMqgLNpH1byjo-Abfmi6rR0OTRWn2FE8dted3w_TI6QC8vGWK2OrJzS_McK9rUj_NRd9aqJqni3hvib',
client_secret: 'EBUTQLsJrW6lKDwd1KJreYQ6EpZK3wuQ1g0KJzLzXFD9rlw2bcr7zp7Xw0dSQdtgFHgjKKGGwaIuT_ql'});
...
router.post('/checkout-process', function(req, res) {
  ...
  let didPaymentSucceed = Math.random();
  ...
  paypal.payment.execute(paymentId, payerId, function(error, payment){
    if(error){
      ...
      if (payment.state == 'approved'){
        console.log(payment);
        var shipA = payment.payer.payer_info.shipping_address;
        var shipping_ad = shipA.line1 + " " + shipA.city + " " + shipA.state + " " + shipA.postal_code + " " + shipA.country_code;
        let newOrder = new Order({ orderID : payment.id, username: req.user.username, address: shipping_ad, orderDate: new Date(), shipping: true});
        newOrder.save();
