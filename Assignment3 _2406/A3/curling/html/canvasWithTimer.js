...
function handleMouseUp(e) {
  ...
  if (shootingCue != null) {
    ...
    //queueCnt = shootingQueue.collection.size();
  }
...
//  if (enableShooting && isHomeClient || isVisitorClient || isSpectatorClient)
//    emitDataBySocket();
  drawCanvas()
}
...
function handleLogoutButton() {
  var type = 0;
  if (isHomeClient) type = 1;
  else if (isVisitorClient) type = 2;
  else type = 3;
  let userRequestObj = {
    action: "Logout",
    type : type
...
$(document).ready(function() {
  ...
  timer = setInterval(handleTimer, 10) //animation timer
...
socket.on('playerData', function(data) {
  ...
  if (playerData.homeStones != null)
    homeStones.setStones(playerData.homeStones)

  if (playerData.visitorStones != null)
    visitorStones.setStones(playerData.visitorStones)

  if (playerData.allStones != null)
    allStones.setStones(playerData.allStones)

  if (playerData.stoneBeingShot != null)
    stoneBeingShot = new Stone(playerData.stoneBeingShot.x, playerData.stoneBeingShot.y, playerData.radius, playerData.colour);
  else stoneBeingShot = null;

  if (playerData.shootingCue != null)
    shootingCue.setCue(playerData.shootingCue);
  else shootingCue = null;

  if (playerData.shootingQueue != null) {

    while (shootingQueue.size() > playerData.shootingQueue.collection.length) {
      shootingQueue.dequeue();
    }
    shootingQueue.setQueue(playerData.shootingQueue);
  }

  /*
  if (playerData.score != null && isHomeClient) {
    score.visitor = playerData.score.visitor;

  } else if (playerData.score != null && isVisitorClient) {
    score.home = playerData.score.home;
  } else if (playerData.score != null) {
    score = playerData.score;
  }
*/

  enableShooting = playerData.enableShooting;
  whosTurnIsIt = playerData.whosTurnIsIt;

  if (playerData.isFinished != null) {
    isFinished = playerData.isFinished;
  } else
    isFinished = false;
  //whosTurnIsIt = playerData.shootingQueue;
  //queueCnt = playerData.queueCnt;
  //while (shootingQueue.size() > queueCnt) {
  //  shootingQueue.dequeue();
  //}
  /*
  if (playerData.shootingQueue != null) {
    shootingQueue.setQueue(shootshootingQueue);
  }
*/
